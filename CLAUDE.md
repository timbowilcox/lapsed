...
Recommendations are evaluated by a scheduled background job (every 6 hours, in vercel.json) and written to a new `insights` table (migration 0016). The table is append-only — state changes write new rows rather than mutating existing ones, preserving full audit history.
